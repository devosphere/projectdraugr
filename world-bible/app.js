(() => {
  ...
  function atlas() {
    ...
    const zoneCards = bible.atlasZones.map(zone => `<article class="zone-card ${zone.kind}" id="zone-${zone.id}"><div><span class="zone-symbol">${bible.atlasLegend.find(item=>item[0]===zone.kind)[1]}</span>${badge(zone.state)}</div><h3>${zone.title}</h3><p class="zone-tags">${zone.tags}</p><p>${zone.scope}</p><p><strong>Placement role:</strong> ${zone.support}</p><small>Requirements: ${zone.issues}</small></article>`).join('');
    ...
    const showZone = id => {
      ...
      selection.innerHTML = `<p class="eyebrow">${zone.tags}</p>${badge(zone.state)}<h3>${zone.title}</h3><p>${zone.scope}</p><p><strong>Supports:</strong> ${zone.support}</p><small>Requirements: ${zone.issues}</small>`;
  ...
  function catalogue() {
    ...
    const showWorld = () => {
      view.innerHTML = `<section class="catalogue-toolbar"><label>Search <input id="search" placeholder="Rabbit, flint, wetland…" /></label><div id="filters">${worldFilters.map(([id,label],index)=>`<button class="${index===0?'selected':''}" data-filter="${id}">${label}</button>`).join('')}</div></section><section class="catalogue-grid" id="catalogue-grid"></section><section class="section issue-register-section"><p class="eyebrow">Live specification index</p><h2>Complete expansion register</h2><p class="section-copy">Every code-named candidate in the approved open issue catalogue is listed below with its owning requirement. A name appearing here is a proposed or specified content candidate—not proof that the runtime already enables it.</p><div class="issue-register" id="issue-register"><p class="register-loading">Loading the GitHub specification register…</p></div></section>`;
      ...
      search.addEventListener('input',render); render(); loadIssueRegister();
    };
    const showRecipes = tabId => {
      ...
      view.innerHTML = `<section class="recipe-intro"><p class="eyebrow">${tab.label}</p><h2>${tab.description}</h2><p>Quantities are written only where this World Bible now defines a concrete design value. <b>Quantity specification pending</b> means the approved issue defines the object and chain but still needs a precise bill of materials before it can be implemented.</p></section><section class="recipe-grid">${records.map(record => recipeCard(record)).join('') || `<p class="empty">The detailed records for this section are being transcribed from their approved specification tickets.</p>`}</section><section class="section issue-register-section"><p class="eyebrow">Specification source</p><h2>All named candidates</h2><p class="section-copy">The live register below retains every code-named candidate from the open catalogue issues, including entries whose individual material quantities are still awaiting their own task-level specification.</p><div class="issue-register" id="issue-register"><p class="register-loading">Loading the GitHub specification register…</p></div></section>`;
      loadIssueRegister();
    };
