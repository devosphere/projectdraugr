-- V237 — story #95 (60 primitive survival equipment entries), slice 1: head, neck and face coverings. Five bare-hand
-- CLOTHING pieces from reachable reed/bark/grass/fibre, each an equippable item with a real anatomy slot and bounded
-- climate benefit, on the V110/V230 garment pattern (item_definition + item_equipment_compatibility + item_source +
-- material_process CRAFT + input + subject). Phrases lead with 'make'/'weave' (CRAFT category terms) and carry no
-- fire/belt/sling/tunic/cloth/shade word, so each classifies CRAFT and routes to its own recipe.
-- (#95's leaf_sunshade is not a worn item here: 'sunshade' is owned by the PLACE_COVER intent (coverKindOf), which
-- already sites a sunshade cover — mapped there at close. smoke_cloth_face_wrap = the existing smoke_face_wrap.)
