...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text, UUID idempotencyKey) {
        ...
        else if (intent == Intent.DRINK) { if(waterInReach(chronicle.location())){physiology.drink(chronicle.id());perception="You drink from the moving water and let the cold settle in your throat.";}else{outcome="FAILED";perception="You look about, but there is no water here fit to drink — no stream, no spring, only dry ground that gives nothing back.";} }
        else if (intent == Intent.WASH) { if(waterInReach(chronicle.location())){physiology.wash(chronicle.id());perception="Cold water runs over your hands and skin, carrying away some of the dirt.";}else{outcome="FAILED";perception="You look for water to wash in, but there is none here — the ground is dry, and nothing runs or stands within reach.";} }
    ...
    private int durationFor(String action, Intent intent) {
        ...
        return switch (intent) { case OBSERVE -> 10; case REST -> 60; case SLEEP -> 480; case GATHER_FIBER, GATHER_STONE, GATHER_BERRIES, GATHER_BRANCHES -> 25; case GATHER_CLAY -> 20; case GATHER_STONE_SLAB -> 30; case GATHER_PLANT -> 20; case FELL_TREE -> 60; case RAID_HIVE -> 15; case COLLECT_INSECTS -> 20; case FISH -> 45; case SNARE -> 25; case TRACK -> 25; case TAME -> 30; case LURE -> 10; case SET_TRAP -> 35; case CHECK_TRAP -> 10; case CRAFT_GARMENT -> 90; case GATHER_MINERAL -> 40; case CRAFT_FIRE_TOOL -> 30; case PROCESS_MATERIAL -> 45; case EAT, DRINK, FEED_FIRE, EXTINGUISH_FIRE -> 5; case WARM_BODY, DRY_BODY, COOL_BODY, SHELTER_BODY, BANK_FIRE -> 10; case STRETCH -> 5; case LIGHT_FIRE -> 20; case COOK_MEAT, TREAT_WOUND, CONFRONT_WILDLIFE, HARVEST_CARCASS -> 10; case EDIT_DOCUMENT, WRITE -> 15; case SKETCH_MAP -> 30; case STRIP_BARK -> 15; case MAKE_CHARCOAL -> 10; case CRAFT_BASKET -> 45; case CRAFT_NET -> 90; case CRAFT_BELT -> 40; case CRAFT_SPEAR, CRAFT_HATCHET -> 35; case CRAFT_FIRE_KIT -> 25; case CRAFT_TINDER -> 10; case CRAFT_DESK, CRAFT_WORKSTATION -> 60; case CRAFT_CHAIR -> 40; case CRAFT_SHELF -> 50; case BUILD_FIRE_PIT, START_LEAN_TO -> 30; case WORK_LEAN_TO -> 45; case DISMANTLE -> 45; case ABANDON_LEAN_TO, RESUME_LEAN_TO -> 5; case MOVE -> 30; case MARK -> 15; case EQUIP, UNEQUIP, DROP -> 5; case DESIGNATE -> 10; case REFINE -> 30; case REPAIR_ITEM -> 20; case ADVANCE_ASSEMBLY -> 45; case INSPECT -> 5; case EXAMINE -> 5; case ANALYZE -> 10; case INVESTIGATE -> 15; case SEARCH -> 10; case LISTEN, SMELL, FEEL -> 5; case READ -> 15; case MEASURE -> 10; case REWORK -> 30; case PERSONAL_ACT -> 20; case AGGRESSION_WILDLIFE -> 5; case AGGRESSION_INANIMATE -> 1; default -> 5; };
    }
    private Intent classify(String action) {
        ...
        if(value.contains("pick up")||value.contains("pick it up")||value.contains("pick them up")||value.contains("pick it back")||value.contains("picked up")||value.contains("grab")||value.contains("retrieve")||value.contains("recover")||value.contains("take back")||value.contains("take it back")||value.contains("fetch")||value.contains("lift the")||value.contains("lift it")||value.contains("lift up")
        ...
        if((usingNet||value.contains("fish")||value.contains("angle")||((value.contains("catch")||value.contains("spear"))&&(value.contains("trout")||value.contains("perch")||value.contains("pike")||value.contains("carp")||value.contains("eel")||value.contains("catfish")||value.contains("crayfish"))))&&!items.actionMatchesProcess(action)) return Intent.FISH;
    ...
    private record ActiveChronicle(UUID id, UUID location) { } private record TravelPlan(UUID destination, int distance, String reason) { } private enum Intent { OBSERVE, MOVE, TRAVEL, MARK, REST, SLEEP, GATHER_FIBER, GATHER_STONE, GATHER_BERRIES, GATHER_BRANCHES, GATHER_CLAY, GATHER_STONE_SLAB, GATHER_PLANT, FELL_TREE, RAID_HIVE, COLLECT_INSECTS, FISH, SNARE, TRACK, TAME, LURE, SET_TRAP, CHECK_TRAP, CRAFT_GARMENT, GATHER_MINERAL, CRAFT_FIRE_TOOL, PROCESS_MATERIAL, SKETCH_MAP, EAT, DRINK, WASH, WARM_BODY, DRY_BODY, COOL_BODY, SHELTER_BODY, STRETCH, TREAT_WOUND, EDIT_DOCUMENT, WRITE, STRIP_BARK, MAKE_CHARCOAL, LIGHT_FIRE, FEED_FIRE, EXTINGUISH_FIRE, BANK_FIRE, COOK_MEAT, CONFRONT_WILDLIFE, HARVEST_CARCASS, CRAFT_BASKET, CRAFT_SPEAR, CRAFT_KNIFE, CRAFT_HAMMER, CRAFT_PICKAXE, CRAFT_HATCHET, CRAFT_FIRE_KIT, CRAFT_TINDER, CRAFT_DESK, CRAFT_CHAIR, CRAFT_SHELF, CRAFT_WORKSTATION, CRAFT_NET, CRAFT_BELT, BUILD_FIRE_PIT, START_LEAN_TO, WORK_LEAN_TO, ABANDON_LEAN_TO, RESUME_LEAN_TO, REPAIR_LEAN_TO, REPAIR_ITEM, DISMANTLE, EQUIP, UNEQUIP, DROP, PICK_UP, STORE, OPEN_CONTAINER, CLOSE_CONTAINER, DESIGNATE, REFINE, ADVANCE_ASSEMBLY, INSPECT, EXAMINE, ANALYZE, INVESTIGATE, SEARCH, LISTEN, SMELL, FEEL, READ, MEASURE, REWORK, URINATE, DEFECATE, PERSONAL_ACT, AGGRESSION_WILDLIFE, AGGRESSION_INANIMATE, UNKNOWN }
    ...
    private String capabilityDomainOf(Intent intent) {
        return switch (intent) {
            case FELL_TREE, GATHER_STONE, GATHER_STONE_SLAB, GATHER_MINERAL, GATHER_CLAY, GATHER_FIBER,
                 ...
                 BUILD_FIRE_PIT, START_LEAN_TO, WORK_LEAN_TO, REPAIR_LEAN_TO, DISMANTLE -> "LOAD";
            case CONFRONT_WILDLIFE, FISH, SNARE, SET_TRAP, CHECK_TRAP, LURE, TAME -> "AIM";
            case OBSERVE, TRACK, INSPECT, EXAMINE, SEARCH, LISTEN, SMELL, FEEL, MEASURE -> "ATTENTION";
    ...
    private Labor laborOf(Intent intent) {
        return switch (intent) {
            ...
            case FELL_TREE, GATHER_MINERAL, GATHER_STONE, GATHER_STONE_SLAB, GATHER_CLAY, HARVEST_CARCASS,
                 ...
                 WORK_LEAN_TO, REPAIR_LEAN_TO, DISMANTLE -> new Labor(12, 8);
            ...
            case GATHER_FIBER, GATHER_BRANCHES, GATHER_BERRIES, GATHER_PLANT, RAID_HIVE, COLLECT_INSECTS,
                 ...
                 CRAFT_DESK, CRAFT_CHAIR, CRAFT_SHELF, CRAFT_WORKSTATION, CRAFT_NET, CRAFT_BELT, REPAIR_ITEM -> new Labor(6, 3);
            // Light acts — looking, marking, writing, handling gear, dressing a wound, tending a fire.
            case OBSERVE, MARK, DESIGNATE, EQUIP, UNEQUIP, DROP, PICK_UP, STORE, OPEN_CONTAINER, CLOSE_CONTAINER, WRITE, EDIT_DOCUMENT, SKETCH_MAP, INSPECT,
    ...
    /** A completed, standing shelter (lean-to) here — for getting out of the weather (#66). */
    private boolean shelterInReach(UUID location) {
        return Boolean.TRUE.equals(jdbc.queryForObject("SELECT EXISTS(SELECT 1 FROM construction_project cp JOIN world_object w ON w.id=cp.object_id WHERE w.current_location_id=? AND cp.project_kind='LEAN_TO' AND cp.state='COMPLETED' AND cp.integrity_percent>0 AND w.lifecycle_state='ACTIVE')", Boolean.class, location));
    }
    /**
