...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text) {
        ...
        else if (text.toLowerCase(Locale.ROOT).contains("lean-to")) { if (construction.startLeanTo(chronicle.id(), chronicle.location(), actionId, resolvedAt)) perception="You mark out a low shelter frame against the weather."; else if (construction.workLeanTo(chronicle.id(), chronicle.location(), resolvedAt)) perception="You bind the shelter frame a little further into place."; else { outcome="FAILED"; perception="The unfinished frame remains as it was."; } }
    ...
    private int durationFor(String action, Intent intent) {
        ...
        return switch (intent) { case OBSERVE -> 10; case REST -> 60; case GATHER_FIBER, GATHER_STONE, GATHER_BERRIES, GATHER_BRANCHES -> 25; case EAT, DRINK, LIGHT_FIRE -> 5; case CONFRONT_WILDLIFE -> 10; case CRAFT_BASKET -> 45; case BUILD_FIRE_PIT -> 30; case MOVE -> 30; default -> 5; };
    }
    private Intent classify(String action) { String value=action.toLowerCase(Locale.ROOT); if(value.contains("wash")||value.contains("bathe")||value.contains("clean myself"))return Intent.WASH; return classifyLegacy(action); }
    ...
    private record ActiveChronicle(UUID id, UUID location) { } private enum Intent { OBSERVE, MOVE, REST, GATHER_FIBER, GATHER_STONE, GATHER_BERRIES, GATHER_BRANCHES, EAT, DRINK, WASH, LIGHT_FIRE, CONFRONT_WILDLIFE, CRAFT_BASKET, BUILD_FIRE_PIT, URINATE, DEFECATE, UNKNOWN }
