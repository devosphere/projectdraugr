...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text, UUID idempotencyKey) {
        ...
        else if (intent == Intent.HARVEST_CARCASS) { WildlifeEncounterService.HarvestResult result=wildlife.harvest(chronicle.id(),chronicle.location(),actionId,resolvedAt); outcome=result.outcome(); perception=result.narration(); if("SUCCEEDED".equals(outcome)) wildlife.recordRefuse(chronicle.location(),15,resolvedAt); }
    ...
    private Intent classify(String action) {
        ...
        if(word(value,"till")||value.contains("plough")||value.contains("plow")||((value.contains("break")||value.contains("turn")||value.contains("work")||value.contains("prepare")||value.contains("hoe"))&&(value.contains("ground")||value.contains("soil")||value.contains("seedbed")||value.contains("seed bed")||value.contains("earth")||value.contains("field")))) return Intent.TILL_GROUND;
        ...
        if(word(value,"reap")||((value.contains("harvest")||value.contains("bring in"))&&(value.contains("crop")||value.contains("the field")))) return Intent.HARVEST_CROP;
        if((value.contains("plant")||value.contains("sow")||value.contains("replant"))&&(value.contains("acorn")||value.contains("pine nut")||value.contains("pine_nut")||value.contains("seed")||value.contains("sapling")||value.contains("seedling")||value.contains("a tree")||value.contains("an oak")||value.contains("a pine")||value.contains("some trees")||value.contains("trees"))) return Intent.PLANT_TREE;
    ...
    private String capabilityDomainOf(Intent intent) {
        return switch (intent) {
            case FELL_TREE, PLANT_TREE, TILL_GROUND, SOW, HARVEST_CROP, GATHER_STONE, GATHER_STONE_SLAB, GATHER_MINERAL, GATHER_CLAY, GATHER_FIBER,
                 GATHER_BRANCHES, GATHER_BERRIES, GATHER_PLANT, FORAGE_GROUND, HARVEST_CARCASS, RAID_HIVE, COLLECT_INSECTS,
                 BUILD_FIRE_PIT, BUILD_FENCE, BUILD_LOOKOUT, BUILD_FUEL_RACK, BUILD_LATRINE, BUILD_TOOL_SHED, BUILD_STORAGE_AREA, RESTORE_HABITAT, START_LEAN_TO, WORK_LEAN_TO, REPAIR_LEAN_TO, REPAIR_STRUCTURE, DISMANTLE, PLACE_WINDBREAK, PLACE_COVER -> "LOAD";
    ...
    private Labor laborOf(Intent intent) {
        return switch (intent) {
            ...
            case FELL_TREE, GATHER_MINERAL, GATHER_STONE, GATHER_STONE_SLAB, GATHER_CLAY, HARVEST_CARCASS, TILL_GROUND,
                 CONFRONT_WILDLIFE, PROCESS_MATERIAL, ADVANCE_ASSEMBLY, BUILD_FIRE_PIT, BUILD_FENCE, BUILD_LOOKOUT, BUILD_FUEL_RACK, BUILD_LATRINE, BUILD_TOOL_SHED, BUILD_STORAGE_AREA, START_LEAN_TO,
            ...
            case GATHER_FIBER, GATHER_BRANCHES, GATHER_BERRIES, GATHER_PLANT, PLANT_TREE, SOW, HARVEST_CROP, FORAGE_GROUND, RAID_HIVE, COLLECT_INSECTS,
