...
public class ChronicleActionService {
    ...
    private final JdbcTemplate jdbc; private final SimulationTickService ticks; private final ChroniclePhysiologyService physiology; private final NarrationPolicy narration; private final PhysicalItemService items; private final CapabilityAdaptationService capability; private final ConstructionService construction; private final ChronicleDiscoveryService discoveries; private final WildlifeEncounterService wildlife; private final FireService fire; private final LiteratureService literature;
    public ChronicleActionService(JdbcTemplate jdbc, SimulationTickService ticks, ChroniclePhysiologyService physiology, NarrationPolicy narration, PhysicalItemService items, CapabilityAdaptationService capability, ConstructionService construction, ChronicleDiscoveryService discoveries, WildlifeEncounterService wildlife, FireService fire, LiteratureService literature) { this.jdbc = jdbc; this.ticks = ticks; this.physiology = physiology; this.narration = narration; this.items=items; this.capability=capability; this.construction=construction; this.discoveries=discoveries; this.wildlife=wildlife; this.fire=fire; this.literature=literature; }
    ...
    public ActionResult resolve(String text) {
        ...
        else if (intent == Intent.GATHER_FIBER) { int bundles=items.gatherPlantFiber(chronicle.id(),chronicle.location(),resolvedAt); outcome=bundles>0?"SUCCEEDED":"FAILED"; perception=bundles>0?"You patiently separate usable plant fiber from the living growth around you.":"You search through the growth, but leave it as it is."; jdbc.update("INSERT INTO chronicle_action_effect (action_id, effect_domain, effect_type, payload) VALUES (?, 'ITEM', 'PLANT_FIBER_GATHERED', jsonb_build_object('bundles', ?))", actionId, bundles); }
        else if (intent == Intent.GATHER_STONE) { int stones=items.gatherFieldStones(chronicle.id(),chronicle.location(),resolvedAt); outcome=stones>0?"SUCCEEDED":"FAILED"; perception=stones>0?"You work loose a few stones from the ground and carry them with you.":"You turn over the ground for a while, then leave it undisturbed."; jdbc.update("INSERT INTO chronicle_action_effect (action_id, effect_domain, effect_type, payload) VALUES (?, 'ITEM', 'FIELD_STONE_GATHERED', jsonb_build_object('stones', ?))", actionId, stones); }
        else if (intent == Intent.GATHER_BERRIES) { int berries=items.gatherWildBerries(chronicle.id(),chronicle.location(),resolvedAt); outcome=berries>0?"SUCCEEDED":"FAILED"; perception=berries>0?"You gather a small handful of ripe berries from the living growth.":"You search the low growth carefully, then let it settle back into place."; jdbc.update("INSERT INTO chronicle_action_effect (action_id,effect_domain,effect_type,payload) VALUES (?,'ITEM','WILD_BERRIES_GATHERED',jsonb_build_object('berries',?))",actionId,berries); }
        else if (intent == Intent.GATHER_BRANCHES) { int branches=items.gatherDryBranches(chronicle.id(),chronicle.location(),resolvedAt); outcome=branches>0?"SUCCEEDED":"FAILED"; perception=branches>0?"You gather a few dry branches from beneath the trees.":"You search the leaf litter for dry wood, then leave with empty hands."; jdbc.update("INSERT INTO chronicle_action_effect (action_id,effect_domain,effect_type,payload) VALUES (?,'ITEM','DRY_BRANCH_GATHERED',jsonb_build_object('branches',?))",actionId,branches); }
        else if (intent == Intent.EAT) { if(items.consumeOne(chronicle.id(),"wild_berries",resolvedAt)){physiology.eat(chronicle.id()); perception="The berries break softly between your teeth, leaving a faint sweetness behind.";} else if(items.consumeOne(chronicle.id(),"raw_game_meat",resolvedAt)){physiology.eat(chronicle.id()); perception="The raw meat is cold and difficult to swallow, but it settles the immediate emptiness.";} else {outcome="FAILED";perception="You search through what you can reach, then lower your hand again.";} }
