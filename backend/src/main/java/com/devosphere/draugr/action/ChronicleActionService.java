...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text, UUID idempotencyKey) {
        ...
        if (contact != null && intent != Intent.MOVE && intent != Intent.TRAVEL && intent != Intent.AGGRESSION_WILDLIFE && intent != Intent.CONFRONT_WILDLIFE) {
            contactAct = com.devosphere.draugr.people.ContactService.recognise(text);
            if (contactAct != null) contactWith = contact.communityInReach(chronicle.location());
            if (contactWith != null) intent = Intent.CONTACT_PEOPLE; else contactAct = null;
        }
