...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text, UUID idempotencyKey) {
        ...
        if (contact != null && intent != Intent.MOVE && intent != Intent.TRAVEL && intent != Intent.AGGRESSION_WILDLIFE && intent != Intent.CONFRONT_WILDLIFE) {
            // Trade first (#113): "offer them my knife for two mats" is an exchange, not a gift, and names more.
            tradeAct = com.devosphere.draugr.people.TradeService.recognise(text);
            contactAct = tradeAct == null ? com.devosphere.draugr.people.ContactService.recognise(text) : null;
            if (tradeAct != null || contactAct != null) contactWith = contact.communityInReach(chronicle.location());
            if (contactWith == null) { tradeAct = null; contactAct = null; }
            else intent = tradeAct != null ? Intent.TRADE_WITH_PEOPLE : Intent.CONTACT_PEOPLE;
        }
