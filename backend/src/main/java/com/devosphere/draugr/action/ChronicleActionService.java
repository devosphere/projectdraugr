...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text) {
        ...
        else if (intent == Intent.CRAFT_SPEAR) { try { items.craftPrimitiveSpear(resolvedAt); perception="You bind stone to a straight branch until a crude spear rests in your hand."; } catch (IllegalStateException ignored) { outcome="FAILED"; perception="The pieces refuse to hold together long enough to become a usable tool."; } }
        else if (intent == Intent.CRAFT_KNIFE) { try { items.craftPrimitiveTool("stone_knife","Stone knife",false,resolvedAt); perception="You work a sharp edge into the stone and bind it into a small hand tool."; } catch (IllegalStateException ignored) { outcome="FAILED"; perception="The stone and loose fiber never settle into a usable edge."; } }
        else if (intent == Intent.CRAFT_HAMMER) { try { items.craftPrimitiveTool("stone_hammer","Stone hammer",true,resolvedAt); perception="You bind the heavy stone to a branch until the head holds firm."; } catch (IllegalStateException ignored) { outcome="FAILED"; perception="The head shifts loose before the tool can hold together."; } }
    ...
    private int durationFor(String action, Intent intent) {
        ...
        return switch (intent) { case OBSERVE -> 10; case REST -> 60; case GATHER_FIBER, GATHER_STONE, GATHER_BERRIES, GATHER_BRANCHES -> 25; case EAT, DRINK, LIGHT_FIRE, FEED_FIRE -> 5; case COOK_MEAT, TREAT_WOUND, CONFRONT_WILDLIFE, HARVEST_CARCASS -> 10; case EDIT_DOCUMENT -> 15; case CRAFT_BASKET -> 45; case CRAFT_SPEAR -> 35; case BUILD_FIRE_PIT, START_LEAN_TO -> 30; case WORK_LEAN_TO -> 45; case ABANDON_LEAN_TO, RESUME_LEAN_TO -> 5; case MOVE -> 30; default -> 5; };
    }
