...
public class ChronicleActionService {
    ...
    public ActionResult resolve(String text, UUID idempotencyKey) {
        ...
        if (contact != null && conductAct == null && intent != Intent.MOVE && intent != Intent.TRAVEL && intent != Intent.AGGRESSION_WILDLIFE && intent != Intent.CONFRONT_WILDLIFE) {
            // Work first (#113): "offer to work for them for two fish" is an agreement, not an exchange of goods.
            agreementAct = agreement == null ? null : com.devosphere.draugr.people.AgreementService.recognise(text);
            // Trade next (#113): "offer them my knife for two mats" is an exchange, not a gift, and names more.
            tradeAct = agreementAct != null ? null : com.devosphere.draugr.people.TradeService.recognise(text);
            contactAct = tradeAct == null && agreementAct == null ? com.devosphere.draugr.people.ContactService.recognise(text) : null;
            if (agreementAct != null || tradeAct != null || contactAct != null) contactWith = contact.communityInReach(chronicle.location());
            if (contactWith == null) { agreementAct = null; tradeAct = null; contactAct = null; }
            else intent = agreementAct == com.devosphere.draugr.people.AgreementService.Act.DO_WORK ? Intent.WORK_FOR_PEOPLE
