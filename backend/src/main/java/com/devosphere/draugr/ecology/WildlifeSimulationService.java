...
public class WildlifeSimulationService {
    ...
    void applyCascades(Instant now) {
        ...
        jdbc.update("UPDATE chunk_flora cf SET quantity = 1, last_harvested_at = ?, established_at = ? " +
            ...
            "    AND nc.world_id = c.world_id AND abs(nc.grid_x - c.grid_x) + abs(nc.grid_y - c.grid_y) = 1)", nowTs, nowTs);
