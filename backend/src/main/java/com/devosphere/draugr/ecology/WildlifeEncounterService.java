...
public class WildlifeEncounterService {
    ...
    public EncounterResult confront(UUID chronicle, UUID chunk, UUID action, Instant at, int tacticBonus) {
        ...
        Combatant body=jdbc.query(
            ...
            "  COALESCE(SUM(CASE WHEN wp.combat_role='JAVELIN' THEN 1 ELSE 0 END),0) javelin," +
            "  COALESCE(SUM(CASE WHEN wp.combat_role='BOW' THEN 1 ELSE 0 END),0) bow," +
            "  COALESCE(SUM(CASE WHEN wp.combat_role='ARROW' THEN 1 ELSE 0 END),0) arrows," +
            // The thrower itself, which is not a weapon and so has no weapon_profile row of its own (#93).
        ...
        // The atlatl (#93): a spear-thrower is not a weapon and has no weapon_profile row, so nothing read it and
        // the whole point of it was missing — an atlatl_dart is catalogued as a JAVELIN and threw exactly as well
        // from a bare hand as from the thrower that exists to launch it. It adds its leverage only when there is
        // something to throw, because a thrower with no dart is a stick.
        //
        ...
        int capability = body.energy()/3 - body.injury()/2 - body.pain()/3 + body.handWeapon()*35 + (body.hardened()>0?10:0) + (body.bronze()>0?18:0) + (body.iron()>0?25:0) + (body.steel()>0?35:0) + (body.blunt()>0?22:0) + (body.javelin()>0?25:0) + (body.atlatl()>0&&body.javelin()>0?12:0) + (archery?40:0) + thrownStones + Math.min(20,body.poison()*20) + tacticBonus + fireEdge;
    ...
    public EncounterResult fish(UUID chronicle, UUID chunk, UUID action, Instant at, String actionText) {
        ...
        else if (items.hasAtLeast(chronicle,"fishing_net",1) || items.hasAtLeast(chronicle,"weir_net_panel",1)) { method="NET"; chance=72; }
