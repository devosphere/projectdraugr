...
public class WildlifeEncounterService {
    ...
    public EncounterResult confront(UUID chronicle, UUID chunk, UUID action, Instant at, int tacticBonus) {
        ...
        int capability = body.energy()/3 - body.injury()/2 - body.pain()/3 + body.handWeapon()*35 + (body.blunt()>0?22:0) + Math.min(10,body.stones()*2) + Math.min(20,body.poison()*20) + tacticBonus;
    ...
    private record Combatant(int energy,int injury,int pain,int handWeapon,int stones,int armour,int poison,int lightShield,int rawhideShield,int blunt){}
