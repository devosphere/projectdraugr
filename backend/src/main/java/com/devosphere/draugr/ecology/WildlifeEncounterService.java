...
public class WildlifeEncounterService {
    ...
    public EncounterResult confront(UUID chronicle, UUID chunk, UUID action, Instant at, int tacticBonus) {
        ...
        if("AERIAL".equals(candidate.movementClass()) && body.stones()==0 && body.javelin()==0)
        ...
        int capability = body.energy()/3 - body.injury()/2 - body.pain()/3 + body.handWeapon()*35 + (body.blunt()>0?22:0) + (body.javelin()>0?25:0) + thrownStones + Math.min(20,body.poison()*20) + tacticBonus;
        ...
        // Armour and shields take part of the blow, graded by what they are (#75, #126): worn scale/shell/war
        // shield turn the most aside; a rawhide shield rather less; a bark or woven-reed shield least of all — an
        // improvised guard is better than a bare arm, but no cover makes a body untouchable, so the worst a
        // mauling can do is only blunted, never erased.
        int blunted = body.armour()*7 + body.rawhideShield()*6 + body.lightShield()*4;
    ...
    private record Combatant(int energy,int injury,int pain,int handWeapon,int stones,int armour,int poison,int lightShield,int rawhideShield,int blunt,int sling,int javelin){}
