...
public class SimulationTickService {
    ...
    public SimulationTick advance() {
        ...
        wildlife.advanceTo(now); items.weatherExposedMetal(now); items.rotExposedOrganics(now); items.slakeExposedGreenware(now); items.advanceCrops(now); items.restPennedDraftBeasts(now); items.advanceDraftHunger(now); items.foulGroundWithLivestock(now);
