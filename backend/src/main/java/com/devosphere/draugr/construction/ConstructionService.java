...
public class ConstructionService {
    ...
    /**
     * Is there an unfinished lean-to frame standing here (#37)?
     *
     * <p>{@code workLeanTo} returns false for three unrelated reasons — no frame, no branches, no fibre — and the
     * caller used to report all three as "the unfinished frame remains as it was", which is untrue of all of them
     * and nonsense when there is no frame at all. This lets the caller tell the first case from the other two.
     */
    @Transactional(readOnly = true) public boolean unfinishedLeanToHere(UUID location) {
        return Boolean.TRUE.equals(jdbc.queryForObject(
            "SELECT EXISTS(SELECT 1 FROM construction_project cp JOIN world_object w ON w.id=cp.object_id " +
            "WHERE w.current_location_id=? AND cp.project_kind='LEAN_TO' AND cp.state IN ('PLANNED','IN_PROGRESS'))",
    ...
    /** Stones a ring takes. Public so the refusal can say the number rather than guess at it (#37). */
    public static final int FIRE_PIT_STONES = 4;
    ...
    public boolean buildFirePit(UUID chronicleId, UUID locationId, UUID actionId, Instant occurredAt) {
        ...
        if (stones.size() < FIRE_PIT_STONES) return false;
        ...
        for (UUID stone : stones.subList(0, FIRE_PIT_STONES)) items.retire(stone, occurredAt, "CONSUMED_FOR_CONSTRUCTION", "field_stone");
