...
public class PhysicalItemService {
    ...
    private void assertCarryCapacity(UUID chronicle) {
        Capacity cap=jdbc.query("SELECT sustained_mass_grams,direct_bulk_ml,maximum_single_lift_grams FROM chronicle_carry_capacity WHERE chronicle_id=?",rs->rs.next()?new Capacity(rs.getInt(1),rs.getInt(2),rs.getInt(3)):null,chronicle);
