...
7. **NEXT (post-M5):** see the audit section below — the natural next thrust is wiring the three AIs (Task #21 AI narration is the first), since the deterministic foundation they sit on is now complete. Plus the smaller enrichments the milestones deliberately deferred (FINE-grade material sources; running the coverage gate automatically in CI).
