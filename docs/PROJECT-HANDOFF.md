...
10. **IN PROGRESS — three-AI integration.** Plan and boundaries in [architecture/three-ai-integration.md](architecture/three-ai-integration.md). Phased: (1) Simulation Agent narration = Task #21, first; (2) Persistent State Architect = authoring-time only (drains `routing_miss_backlog` into reviewed migrations); (3) Persistent State Auditor = read-only summaries.
    ...
    - **NEXT — Phase 1b: wire into `ChronicleActionService.resolve()`.** After `perception` is final and the frame built: `if (narrationRouter.shouldUseAI(...)) perception = simulationNarrator.refine(frame, perception);` then persist/return the refined string. One edit, behind the gate. Then 1c: live verification with a real key on a routed moment (kill / death / HIGH-attention observe).
    - **Operator note / open decision:** the model defaults to `claude-opus-5` (we don't silently downgrade). For this high-frequency, low-stakes narration-refinement use, `claude-haiku-4-5` is likely the better latency/cost choice — set `DRAUGR_AI_MODEL`. Live use needs the user's `ANTHROPIC_API_KEY`.
