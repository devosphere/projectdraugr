...
5. **NEXT:** M3b (graded quality — inspect/rework, quality flowing input→output), M4 (most process/item vocabulary already shipped in V57; what remains is the cut-planning multi-output schema extension and wiring building materials to consume stages), M5 (coverage as standing measure — promote `routing-reachability-probe.sql` into a build-time harness). Also Task #21 (AI narration).
