...
6. **NEXT:** M4 (most process/item vocabulary already shipped in V57; what remains is the cut-planning multi-output `material_process` form and wiring building materials to be consumed by stages — both sit on the V58 schema), M5 (coverage as standing measure — promote `routing-reachability-probe.sql` into a build-time harness). Also Task #21 (AI narration).
