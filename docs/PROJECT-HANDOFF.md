...
10. **NEXT:** wiring the three AIs — Task #21 (AI narration, the Simulation Agent's voice) is the first and is credential-gated (needs an LLM API key; the seam `NarrationRouter`/`NarrationEngine` is built). Still deferred: FINE-grade material sources, and the `SALT_DEPOSIT` world-generator placement.
